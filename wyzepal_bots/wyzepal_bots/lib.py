...
from wyzepal import Client, WyzePalError
from wyzepal_bots.custom_exceptions import ConfigValidationError
    ...
    def __init__(
        ...
        try:
            ...
            logging.error('Cannot fetch user profile, make sure you have set'
                          ' up the wyzepalrc file correctly.')
            sys.exit(1)
...
def is_private_message_from_another_user(message_dict: Dict[str, Any], current_user_id: int) -> bool:
    ...
    wyzepal/wyzepal project, so refactor with care.  See the comments in
...
def run_message_handler_for_bot(
    ...
    # Make sure you set up your ~/.wyzepalrc

    client_name = "WyzePal{}Bot".format(bot_name.capitalize())
