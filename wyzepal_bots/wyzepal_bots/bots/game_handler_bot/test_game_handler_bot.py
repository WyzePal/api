...
from wyzepal_bots.game_handler import GameInstance
...
```move <column-number>```'''
    ...
    def test_exception_handling(self) -> None:
        with patch('logging.exception'), \
                patch('wyzepal_bots.game_handler.GameAdapter.command_quit',
    ...
    def test_game_over_and_leaderboard(self) -> None:
        bot = self.setup_game()
        bot.put_user_cache()
        with patch('wyzepal_bots.bots.game_handler_bot.game_handler_bot.MockModel.determine_game_over', return_value='foo@example.com'):
 ...
 **test-bot** | 0 | 0 | 0 | 0'
    ...
    def test_current_turn_winner(self) -> None:
        bot = self.setup_game()
        with patch('wyzepal_bots.bots.game_handler_bot.game_handler_bot.MockModel.determine_game_over', return_value='current turn'):
            self.verify_response('move 3', '!avatar(foo@example.com) **foo** won! :tada:',
                                 1, bot=bot, stream='test', subject='test game')
    ...
    def test_computer_turn(self) -> None:
        ...
        self.verify_response('move 3', '**foo** moved in column 3\n\nfoo\n\n!avatar(test-bot@example.com) It\'s **test-bot**\'s (:red_circle:) turn.',
                             0, bot=bot, stream='test', subject='test game')
        with patch('wyzepal_bots.bots.game_handler_bot.game_handler_bot.MockModel.determine_game_over', return_value='test-bot@example.com'):
    ...
    def test_computer_endgame_responses(self) -> None:
        ...
        with patch('wyzepal_bots.bots.game_handler_bot.game_handler_bot.MockModel.determine_game_over', return_value='foo@example.com'):
            self.verify_response('move 5', 'You won! Nice!',
                                 2, bot=bot, stream='test', subject='test game')
        bot = self.setup_computer_game()
        with patch('wyzepal_bots.bots.game_handler_bot.game_handler_bot.MockModel.determine_game_over', return_value='draw'):
