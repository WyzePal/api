...
import os
import sys
import wyzepal_bots.run
from wyzepal_bots.lib import extract_query_without_mention
import unittest
...
class TestDefaultArguments(TestCase):
    ...
    @patch('sys.argv', ['wyzepal-run-bot', 'giphy', '--config-file', '/foo/bar/baz.conf'])
    @patch('wyzepal_bots.run.run_message_handler_for_bot')
    def test_argument_parsing_with_bot_name(self, mock_run_message_handler_for_bot: mock.Mock) -> None:
        with patch('wyzepal_bots.run.exit_gracefully_if_wyzepal_config_file_does_not_exist'):
            wyzepal_bots.run.main()
    ...
    @patch('sys.argv', ['wyzepal-run-bot', path_to_bot, '--config-file', '/foo/bar/baz.conf'])
    @patch('wyzepal_bots.run.run_message_handler_for_bot')
    ...
    def test_adding_bot_parent_dir_to_sys_path_when_bot_name_specified(self) -> None:
        ...
        expected_bot_dir_path = os.path.join(
            os.path.dirname(wyzepal_bots.run.__file__),
            'bots',
            bot_name
        )
    ...
    def _test_adding_bot_parent_dir_to_sys_path(self, bot_qualifier, bot_dir_path):
        # type: (str, str) -> None
        with patch('sys.argv', ['wyzepal-run-bot', bot_qualifier, '--config-file', '/path/to/config']):
            with patch('wyzepal_bots.finder.import_module_from_source', return_value=mock.Mock()):
                with patch('wyzepal_bots.run.run_message_handler_for_bot'):
                    with patch('wyzepal_bots.run.exit_gracefully_if_wyzepal_config_file_does_not_exist'):
                        wyzepal_bots.run.main()
    ...
    def test_run_bot_by_module_name(self, mock_os_path_isfile: mock.Mock) -> None:
        ...
        with patch('sys.argv', ['wyzepal-run-bot', 'bot.module.name', '--config-file', '/path/to/config']):
