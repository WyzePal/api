...
class IRCBot(irc.bot.SingleServerIRCBot):
    ...
    def __init__(self, wyzepal_client, stream, topic, channel,
    ...
    def wyzepal_sender(self, sender_string):
        # type: (str) -> str
    ...
    def on_welcome(self, c, e):
        ...
        def forward_to_irc(msg):
            ...
            not_from_wyzepal_bot = msg["sender_email"] != self.wyzepal_client.email
            if not not_from_wyzepal_bot:
                # Do not forward echo
                return
        ...
        z2i = mp.Process(target=self.wyzepal_client.call_on_each_message, args=(forward_to_irc,))
        z2i.start()

    def on_privmsg(self, c, e):
        # type: (ServerConnection, Event) -> None
        content = e.arguments[0]
        sender = self.wyzepal_sender(e.source)
        if sender.endswith("_wyzepal@" + self.IRC_DOMAIN):
            return
